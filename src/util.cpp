...
#else
    const char* pszModule = "kizombacoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "kizombacoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kizombacoind.pid"));
