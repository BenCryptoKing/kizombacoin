...
{
    string strBlockFn = strprintf("kmc%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile kmcdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && kmcdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(kmcdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), kmcdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
...
{
    RenameThread("kizombacoin-loadkmc");
