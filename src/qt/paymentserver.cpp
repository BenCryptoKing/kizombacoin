...
const int ANDROMEDA_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ANDROMEDA_IPC_PREFIX("kizombacoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(ANDROMEDA_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ANDROMEDA_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(ANDROMEDA_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start kizombacoin: click-to-pay handler");
    else
