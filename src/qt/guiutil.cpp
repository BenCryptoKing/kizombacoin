...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("kizombacoin"))
        return false;
...
{
    // Convert kizombacoin:// to kizombacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kizombacoin://"))
    {
        uri.replace(0, 12, "kizombacoin:");
    }
...
{
    return GetAutostartDir() / "kizombacoin.desktop";
}
    ...
    QMessageBox(parent)
{
    header = tr("BlackCoin-Qt") + " " + tr("version") + " " +
        ...
        tr("Usage:") + "\n" +
        "  kizombacoin-qt [" + tr("command-line options") + "]                     " + "\n";
